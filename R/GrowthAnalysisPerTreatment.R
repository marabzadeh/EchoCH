...
#' @return PerTreatmentGrowth.df : dataframe reporting the VAF-changes-per-month for each treatment and other statistics
#' @examples
...
GrowthAnalysisPerTreatment <- function(PerVarGrowth.df, time_threshold){
  ...
  #gene.ls <- vector (mode = "character", length = ran[2]  )
  trt.ls <- vector (mode = "character", length = ran[2]  )
  mean.ls <- vector (mode = "integer", length = ran[2]  )
  median.ls <- vector (mode = "integer", length = ran[2]  )
  min.ls <-   vector (mode = "integer", length = ran[2]  )
  max.ls <-   vector (mode = "integer", length = ran[2]  )
  SD.ls <-   vector (mode = "integer", length = ran[2]  )
  SE.ls <-   vector (mode = "integer", length = ran[2]  )
  mutNum.ls <-vector (mode = "integer", length = ran[2]  )
  ptNum.ls <-vector (mode = "integer", length = ran[2]  )
  meanSE <-vector (mode = "integer", length = ran[2]  )
  datapoints <-vector (mode = "list", length = ran[2]  )
  ...
  res.df <- data.frame( trt.ls, mean.ls, median.ls, min.ls, max.ls, SD.ls, SE.ls, ptNum.ls, mutNum.ls, meanSE, datapoints)
  #res.df <- res.df[res.df$gene.ls != "",]

  res.df <- res.df[order(res.df$meanSE, decreasing = T), ]

  colnames (res.df) <- c("treatment",	"mean_CHvaf_change_per_month",	"median_CHvaf_change_per_month",
                         "min_CHvaf_change_per_month"	, "max_CHvaf_change_per_month",
                         "standard_deviation",	"standard_error",	"number_patients",	"number_mutations",
                         "meanSE", "datapoints")

  PerTreatmentGrowth.df <- res.df
  return(PerTreatmentGrowth.df)
